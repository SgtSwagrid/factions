package factions.events.flags;

import static factions.util.Colour.*;
import static factions.util.MathUtils.*;
import static factions.util.Messenger.*;
import static factions.util.Teams.*;
import static factions.util.Values.*;
...
public class FlagsEvent {
	...
	private static final int FURTHEST_CENTRE = 250;
	...
	public static void start() {
		...
		for(Team team : getTeams()) {
			...
			Colour teamColour = fromFormatter(team.getColor());
			BlockPos pos = getPosition("f:flag_position(" + teamColour.UNLOCALISED_NAME + ")");
			announce("The &" + teamColour.DISPLAY_NAME + " Flag& has been discoved at "
					+ posStr(pos) + ".", teamColour);
		}
		//Register and announce the flags event as being active.
		setBoolean("f:flags_active", true);
		pushScore("Purge Score");
		announce("The purge has begun. Flags can now be captured.");
	}
		...
		for(Team team1 : getTeams()) {
			for(Team team2 : getTeams()) {
				if(!team1.isSameTeam(team2)) {
					...
					Values.setInteger("f:num_captures(" + colour1 + "," + colour2 + ")", 0);
				}
			}
		}
		
		//Register and announce the flags event as having ended.
		setBoolean("f:flags_prepared", false);
		setBoolean("f:flags_active", false);
		popScore();
		announce("The purge has ended.");
	}
	...
	private static void spawnFlag(World world, Colour colour, int x, int z) {
		...
		setPosition("f:flag_home(" + colour.UNLOCALISED_NAME + ")", pos);
		setPosition("f:flag_position(" + colour.UNLOCALISED_NAME + ")", pos);
		setInteger("f:flag_dimension(" + colour.UNLOCALISED_NAME + ")", 0);
		setBoolean("f:flag_in_world(" + colour.UNLOCALISED_NAME + ")", true);
	}
	...
	private static void removeFlags() {
		...
		for(Team team : getTeams()) {
			...
			if(getBoolean("f:flag_in_world(" + colour.UNLOCALISED_NAME + ")")) {
				
				//Remove the flag from the world.
				BlockPos position = getPosition("f:flag_position(" + colour.UNLOCALISED_NAME + ")");
				int dimension = getInteger("f:flag_dimension(" + colour.UNLOCALISED_NAME + ")");
				Flag.remove(DimensionManager.getWorld(dimension), position);
				setBoolean("f:flag_in_world(" + colour.UNLOCALISED_NAME + ")", false);
			}
		}
		...
		for(EntityPlayer player : world.getPlayers(EntityPlayer.class, p -> true)) {
			Values.setBoolean("f:has_flag(" + player.getName() + ")", false);
		}
	}
	 ...
	 */
	static void interact(Colour flagColour, EntityPlayer player,
	...
	private static void interactEnemy(Colour flagColour, Team flagTeam, EntityPlayer player,
		...
		} else if(getInteger("f:num_captures(" + playerColour.UNLOCALISED_NAME + ","
	...
	private static void pickUp(Colour flagColour, Team flagTeam, EntityPlayer player,
		...
		setEnum("f:held_flag(" + player.getName() + ")", flagColour);
		setBoolean("f:has_flag(" + player.getName() + ")", true);
		setBoolean("f:flag_in_world(" + flagColour.UNLOCALISED_NAME + ")", false);
	...
	private static void interactFriendly(Colour colour, Team team,
		...
		//Capture an enemy flag.
			...
			Colour capturedColour = getEnum("f:held_flag(" + player.getName() + ")", Colour.class);
	...
	private static void capture(Colour colour, Team team, EntityPlayer player,
		...
		BlockPos capturedHome = getPosition("f:flag_home(" + capturedColour.UNLOCALISED_NAME + ")");
		...
		setBoolean("f:has_flag(" + player.getName() + ")", false);
		setBoolean("f:flag_in_world(" + capturedColour.UNLOCALISED_NAME + ")", true);
		setPosition("f:flag_position(" + capturedColour.UNLOCALISED_NAME + ")", returnPos);
		setInteger("f:flag_dimension(" + capturedColour.UNLOCALISED_NAME + ")", 0);
		
		//Get the number of times this team has captured this particular flag, increasing it by 1.
		int captures = increment("f:num_captures(" + colour.UNLOCALISED_NAME
	...
	private void dropFlag(EntityPlayer player) {
		...
		if(getBoolean("f:has_flag(" + player.getName() + ")")) {
			...
			Colour flagColour = getEnum("f:held_flag(" + player.getName() + ")", Colour.class);
			...
			setBoolean("f:has_flag(" + player.getName() + ")", false);
			setBoolean("f:flag_in_world(" + flagColour.UNLOCALISED_NAME + ")", true);
			setPosition("f:flag_position(" + flagColour.UNLOCALISED_NAME + ")", position);
			setInteger("f:flag_dimension(" + flagColour.UNLOCALISED_NAME + ")", player.dimension);
	...
	public void onPlayerJoin(PlayerEvent.PlayerLoggedInEvent event) {
		...
		if(getBoolean("f:flags_active")) {
			...
			for(WorldServer world : DimensionManager.getWorlds()) {
				...
				for(EntityPlayer player : world.getPlayers(EntityPlayer.class,
						p -> getBoolean("f:has_flag(" + p.getName() + ")"))) {
					
					Colour flagColour = getEnum("f:held_flag(" + player + ")", Colour.class);
					Colour playerColour = fromFormatter(player.getTeam().getColor());
		...
		} else if(getBoolean("f:flags_prepared") && event.player.getTeam() != null) {
